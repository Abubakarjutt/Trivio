  ...
  return (
    <div className="flex min-h-screen">
      ...
      >
          ...
          {[
            ...
            <span
              ...
              style={{
                ...
                animation: `floatAmount ${8 + i * 1.5}s ease-in-out infinite ${i * 0.8}s`,
              }}
            >
              {item.text}
            </span>
        ...
        <style>{`
          @keyframes floatAmount { 0%, 100% { transform: translateY(0); } 50% { transform: translateY(-12px); } }
        ...
        <div className="relative flex flex-col justify-between h-full p-12 xl:p-16">
          ...
          <div className="space-y-10">
            ...
            <div className="space-y-4">
              {METRICS.map((m, i) => (
                <div key={i} className="flex items-baseline gap-3" style={{ animation: `slideUp 0.6s cubic-bezier(0.16,1,0.3,1) both ${m.delay}` }}>
                  <span className="text-3xl font-bold tabular-nums" style={{ color: "rgba(42,138,90,0.95)" }}>{m.value}</span>
                  <span className="text-white/40 text-sm">{m.label}</span>
                </div>
              ))}
            </div>
            <div className="rounded-2xl p-5" style={{ background: "rgba(255,255,255,0.04)", boxShadow: "inset 0 0 0 1px rgba(255,255,255,0.07)" }}>
      ...
      <div className="flex flex-1 flex-col bg-white">
        ...
        <div className="flex items-center justify-between px-8 py-5 border-b border-slate-100">
          ...
          <Link href="https://trivio-ai.com" className="flex items-center gap-1.5 text-xs font-medium text-slate-400 hover:text-slate-700 transition-colors group">
            ...
            {submitted ? (
              ...
              <div className="text-center py-8">
                <div className="flex justify-center mb-6">
                  <div className="h-16 w-16 rounded-2xl flex items-center justify-center" style={{ background: "rgba(26,102,68,0.08)" }}>
                    <MailCheck className="h-8 w-8" style={{ color: "#1A6644" }} />
                  </div>
                </div>
                ...
                <Link
                  href="/login"
                  className="inline-flex items-center justify-center h-11 px-6 rounded-xl border-2 border-slate-200 text-slate-700 text-sm font-semibold hover:border-green-600 hover:text-green-700 transition-all"
                >
                  Go to sign in
                </Link>
              </div>
              ...
              <>
                <div className="mb-10">
                  <h2 className="text-4xl font-bold text-slate-900 leading-tight">Create your account</h2>
                  <p className="mt-2 text-slate-500 text-base">Free to start — no credit card required.</p>
                </div>
                ...
                <form onSubmit={handleSubmit} className="space-y-5">
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    <div className="space-y-2">
                      ...
                      <Input id="name" placeholder="Jane Smith" value={form.name} onChange={(e) => setForm((f) => ({ ...f, name: e.target.value }))} required autoComplete="name" className="h-12 text-base border-slate-200" />
                    </div>
                    <div className="space-y-2">
                      <Label htmlFor="email" className="text-xs font-semibold uppercase tracking-widest text-slate-400">Email address</Label>
                      <Input id="email" type="email" placeholder="you@example.com" value={form.email} onChange={(e) => setForm((f) => ({ ...f, email: e.target.value }))} required autoComplete="email" className="h-12 text-base border-slate-200" />
                    </div>
                  </div>

                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    <div className="space-y-2">
                      <Label htmlFor="password" className="text-xs font-semibold uppercase tracking-widest text-slate-400">Password</Label>
                      <Input id="password" type="password" placeholder="At least 8 characters" value={form.password} onChange={(e) => setForm((f) => ({ ...f, password: e.target.value }))} required minLength={8} autoComplete="new-password" className="h-12 text-base border-slate-200" />
                      <PasswordStrength password={form.password} />
                    </div>
                    <div className="space-y-2">
                      <Label htmlFor="confirmPassword" className="text-xs font-semibold uppercase tracking-widest text-slate-400">Confirm password</Label>
                      <Input id="confirmPassword" type="password" placeholder="••••••••" value={form.confirmPassword} onChange={(e) => setForm((f) => ({ ...f, confirmPassword: e.target.value }))} required autoComplete="new-password" className="h-12 text-base border-slate-200" />
                    </div>
                  </div>

                  <div className="flex items-start gap-3 pt-1">
                    <input id="gdprConsent" type="checkbox" checked={gdprConsent} onChange={(e) => setGdprConsent(e.target.checked)} className="mt-0.5 h-4 w-4 rounded border-slate-300 accent-green-700 cursor-pointer shrink-0" />
                    <label htmlFor="gdprConsent" className="text-xs text-slate-500 leading-relaxed cursor-pointer">
                      I have read and agree to the{" "}
                      <Link href="/privacy" className="underline underline-offset-2 hover:text-slate-700 font-medium">Privacy Policy</Link>
                      {" "}and consent to the processing of my personal data to provide this service.
                    </label>
                  </div>

                  <div className="pt-1">
                    <Button type="submit" disabled={register.isPending || !gdprConsent} className="w-full h-12 text-base font-semibold rounded-xl" style={{ background: "#1A6644" }}>
