...
export default function ARAgingPage() {
  const { data, isLoading } = trpc.invoices.arAging.useQuery();
  ...
  return (
    <div className="min-h-full">
      <div className="sticky top-0 z-10 border-b border-border/40 backdrop-blur-sm bg-background/95 px-8 py-4">
        <h1 className="font-serif text-2xl font-medium text-foreground leading-tight">AR Aging</h1>
        <p className="text-xs text-muted-foreground mt-0.5">Outstanding customer balances grouped by days past due</p>
      </div>
          ...
          <Card>
            <CardHeader>
              <div className="flex items-center justify-between">
                ...
                <span className="text-sm text-muted-foreground">{data.invoiceCount} outstanding invoice{data.invoiceCount !== 1 ? "s" : ""}</span>
              </div>
            </CardHeader>
