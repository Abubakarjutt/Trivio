...
import { ArrowLeft, Loader2, Plus, Trash2, Sparkles, AlertTriangle, CheckCircle2, XCircle } from "lucide-react";
...
const EMPTY_LINE: LineItem = { description: "", quantity: "1", unitPrice: "", taxRateCode: "", taxAmount: "0" };
...
function NewBillForm() {
  ...
  const { data: contacts = [] } = trpc.contacts.list.useQuery({ type: "SUPPLIER" });
  ...
  const create = trpc.bills.create.useMutation({
    onSuccess: (bill) => { toast({ title: "Bill created" }); router.push(`/bills/${bill.id}`); },
  ...
  const updateLine = useCallback((idx: number, field: keyof LineItem, value: string) => {
    setLines((prev) => {
      ...
      if (field === "quantity" || field === "unitPrice" || field === "taxRateCode") {
        ...
        line.taxAmount = rate ? (qty * price * (Number(rate.rate) / 100)).toFixed(2) : "0";
      }
      next[idx] = line;
      return next;
    });
    ...
    <div className="p-6 lg:p-8 max-w-4xl mx-auto space-y-5">
      <div className="flex items-center gap-3">
        ...
        <div>
          ...
          <p className="text-muted-foreground text-sm mt-0.5">Record a supplier bill</p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-5">
        <div className="md:col-span-2 space-y-5">
          ...
          <Card>
            ...
            <CardContent className="space-y-3">
              <div className="grid grid-cols-12 gap-2 text-xs font-semibold uppercase tracking-wide text-muted-foreground/70 pb-1 border-b">
              ...
              {lines.map((line, idx) => (
                <div key={idx} className="grid grid-cols-12 gap-2 items-start">
                  ...
                  <div className="col-span-2">
                    <Input type="number" min="0" step="0.01" className="text-right" value={line.quantity} onChange={(e) => updateLine(idx, "quantity", e.target.value)} />
                  </div>
                  <div className="col-span-2">
                    <Input type="number" min="0" step="0.01" className="text-right" placeholder="0.00" value={line.unitPrice} onChange={(e) => updateLine(idx, "unitPrice", e.target.value)} />
                  </div>
                  <div className="col-span-2">
                    <Select value={line.taxRateCode} onValueChange={(v) => updateLine(idx, "taxRateCode", v)}>
                      ...
                      <SelectContent>
                        ...
                        {taxRates.map((r) => (
                          <SelectItem key={r.code} value={r.code}>{r.name} ({Number(r.rate)}%)</SelectItem>
                        ))}
                      </SelectContent>
                    </Select>
                  </div>
                  ...
                  <div className="col-span-1 flex justify-center">
                    {lines.length > 1 && (
                      <Button variant="ghost" size="icon" className="h-8 w-8 text-muted-foreground hover:text-destructive" onClick={() => setLines((p) => p.filter((_, i) => i !== idx))}>
              ...
              <div className="space-y-1.5">
                ...
                <Input placeholder="Payment terms, reference, etc." value={notes} onChange={(e) => setNotes(e.target.value)} />
              </div>
            </CardContent>
          </Card>
        </div>
        ...
        <div className="space-y-5">
          ...
          <div className="flex flex-col gap-2">
            ...
            <Button variant="outline" asChild><Link href="/bills">Cancel</Link></Button>
          </div>
        </div>
      </div>
    </div>
