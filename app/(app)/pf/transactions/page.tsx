...
type PendingBatch = {
  batchId: string;
  items: { date: string | Date; description: string; amount: number }[];
};
    ...
    <div className="flex flex-col gap-6 p-6">
      ...
      <div className="grid grid-cols-2 gap-3 sm:grid-cols-4">
        {[
          {
            label: "Total Debits",
            value: summary ? fmt(summary.totalDebits) : "—",
            icon: TrendingDown,
            color: "text-red-500",
          },
          {
            label: "Total Credits",
            value: summary ? fmt(summary.totalCredits) : "—",
            icon: TrendingUp,
      ...
      {isLoading ? (
          ...
          {month ? (
            <>
              ...
              <p className="text-muted-foreground/60 mt-1 text-xs">
                Your data may be in a different year — use &laquo; to jump back
              </p>
              <Button
                size="sm"
                variant="outline"
            ...
            <>
              ...
              <p className="text-muted-foreground/60 mt-1 text-xs">
                Import a bank statement, or add one manually
              </p>
        ...
        <>
                ...
                {data.items.map((txn) => (
                  ...
                  >
                    ...
                    <td className="px-4 py-3">
                      ...
                      <div className="text-muted-foreground/60 max-w-xs truncate text-xs">
                        {txn.description}
                      </div>
                    </td>
                    <td className="px-4 py-3">
                    ...
                    <td
                      className={`px-4 py-3 text-right font-medium tabular-nums ${txn.type === "DEBIT" ? "text-red-500" : "text-emerald-500"}`}
                    >
                      {txn.type === "DEBIT" ? "−" : "+"}
          ...
          {/* Mobile card list */}
          <div className="bg-card flex flex-col divide-y overflow-hidden rounded-lg border md:hidden">
            {data.items.map((txn) => (
              <TransactionCard
                key={txn.id}
